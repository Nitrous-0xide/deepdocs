...
## Table of Contents
- [Overview](#overview)
- [Loader Functions (Ldr*)](#loader-functions-ldr)
- [Run-Time Library Functions (Rtl*)](#run-time-library-functions-rtl)
- [System Service Dispatch Stubs (Nt\*/Zw\*)](#system-service-dispatch-stubs-ntzw)
- [Debugging Functions (Dbg/DbgUi)](#debugging-functions-dbgdbgui)
- [Windows Subsystem Communication (Csr\*)](#windows-subsystem-communication-csr)
- [Event Tracing Functions (Etw\*)](#event-tracing-functions-etw)
- [Thread Pool Functions (Tp\*)](#thread-pool-functions-tp)
- [C Runtime Functions](#c-runtime-functions)
- [Data Exports](#data-exports)
- [Discontinued Functions](#discontinued-functions)
- [Detailed Documentation: RtlSetProcessIsCritical](#detailed-documentation-rtlsetprocessiscritical)
...
| Function                        | Description           |
| ------------------------------- | ---------------------|
| `RtlCompressBuffer`              | Compresses a buffer  |
...
These functions are the user-mode entry points to kernel system services. Each `Nt*` function has a corresponding `Zw*` function that behaves identically in user mode. They perform system calls via syscall stubs.
...
| NtCreateUserProcess       | Creates a new user process        | Not reliably exported           |
...
| Function                         | Description                         | Documentation Status  |
| --------------------------------- | ----------------------------------- | ---------------------|
| NtOpenProcessToken                | Opens process token                 | Documented for Win2000+ |
| NtOpenThreadToken                 | Opens thread token                  | Variable                |
| NtDuplicateToken                  | Duplicates a token                  | Undocumented until 2008-2009 |
| NtAdjustPrivilegesToken           | Adjusts token privileges            | Documented for Win2000+ |
...
| NtGetPlugPlayEvent       | Gets PnP events (disc. 6.2)|
...
| Function              | Description                                           |
| --------------------- | -----------------------------------------------------|
| DbgBreakPoint         | Software breakpoint; implementation of DebugBreak (3.51–6.0); modern compilers use `__debugbreak` intrinsic |
...
| CsrAllocateCapturePointer     | Allocates capture pointer (discontinued 5.0)   | Discontinued                 |
...
| CsrClientMaxMessage           | Gets max message size (discontinued 4.0)       | Discontinued                 |
| CsrClientSendMessage          | Sends message to CSRSS (discontinued 4.0)      | Discontinued                 |
| CsrClientThreadConnect        | Connects thread to CSRSS (discontinued 4.0)    | Discontinued                 |
| CsrFreeCaptureBuffer          | Frees capture buffer                           |                              |
| CsrIdentifyAlertableThread    | Identifies alertable thread                    |                              |
| CsrNewThread                  | Notifies CSRSS of new thread (discontinued 6.0 SP1) | Discontinued            |
| CsrProbeForRead               | Probes buffer for read access (discontinued 6.0)| Discontinued                |
| CsrProbeForWrite              | Probes buffer for write access (discontinued 6.0)| Discontinued               |
| CsrSetPriorityClass           | Sets CSRSS thread priority class               |                              |
| CsrpProcessCallbackRequest    | Processes callback request (discontinued 4.0)  | Discontinued                 |
...
| wcsncat      | Wide-character limited concat      |
...
Sets or clears the critical status of the current process. A critical process, if terminated, causes the system to crash with a bugcheck (BSOD). This function is used internally by essential system processes such as `csrss.exe` and `wininit.exe`.
...
Requires SeDebugPrivilege (typically needs administrative privileges)
...
- [Geoff Chappell - Loader Functions](https://www.geoffchappell.com/studies/windows/win32/ntdll/api/ldr/index.htm)
- [Geoff Chappell - NTDLL 3.51 Exports](https://www.geoffchappell.com/studies/windows/win32/ntdll/history/exports351.htm)
- [Geoff Chappell - NTDLL 3.50 Exports](https://www.geoffchappell.com/studies/windows/win32/ntdll/history/exports350.htm)
- [Geoff Chappell - KERNEL32 6.0 Exports](https://www.geoffchappell.com/studies/windows/win32/kernel32/history/exports60.htm)
- [NtDoc - TIME_FIELDS documentation](https://ntdoc.m417z.com/#TIME_FIELDS)
- [GitHub - ntdll-usermode-ram-read-write](https://github.com/rprn12/ntdll-usermode-ram-read-write)
- [GitHub - NtDll header-only library](https://github.com/Cr4sh/NTDLL)
- [GitHub - Windows Subsystems Documentation](https://github.com/MicrosoftDocs/windows-uwp)

---

## Disclaimer

This documentation is based on reverse engineering and community research. Function signatures, behaviors, and availability may vary between Windows versions and updates. Microsoft does not provide support for these APIs and their use may void compatibility guarantees.
